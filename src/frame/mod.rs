...
impl FrameHead {
    ...
    /// otherwise a `NotEnoughCapacity` error will be generated.
    ...
    /// Parse from provided buffer, returns the count of read bytes.
    pub fn decode(buf: &[u8]) -> Result<(Self, usize), FrameError> {
        ...
        match length {
            ...
            PayloadLen::Extended1(_) => {
                ...
                length = PayloadLen::from_byte2(unsafe {
                    *slice_to_array::<_, 2>(slice(buf, 2, 4))
                });
            ...
            PayloadLen::Extended2(_) => {
                ...
                length = PayloadLen::from_byte8(unsafe {
                    *slice_to_array::<_, 8>(slice(buf, 2, 10))
                });
        ...
        match mask {
            ...
            _ => {
                ...
                let key =
                    *unsafe { slice_to_array::<_, 4>(slice(buf, n, n + 4)) };
...
mod test {
    ...
    fn frame_head() {
        ...
        for head in [head, head2] {
            ...
            let (head2, decode_n) =
                FrameHead::decode(&buf[0..encode_n + 128]).unwrap();

            assert_eq!(encode_n, decode_n);
            assert_eq!(head, head2);

            let mut buf2 = vec![0; 1024];
            let encode_n2 = head2.encode(&mut buf2).unwrap();
